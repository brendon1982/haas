...
export abstract class BaseSignalRService {
  ...
  constructor(hubUrl: string, protected logPrefix: string) {
  ...
  public startConnection(): void {
    if (this.hubConnection.state === signalR.HubConnectionState.Disconnected) {
      this.hubConnection
        .start()
        .then(() => {
          console.log(`${this.logPrefix} SignalR Connection started`);
          this._connectionState.set(signalR.HubConnectionState.Connected);
          this.onConnected();
        })
        .catch(err => {
          console.log(`Error while starting ${this.logPrefix} SignalR connection: ` + err);
          this._connectionState.set(signalR.HubConnectionState.Disconnected);
        });
    }
  }
